...
import 'package:workout_helper/pages/exercise/training_plan_detail.dart';
    ...
    showDialog<String>(
      ...
      if (_selectedExercises.length > 0) {
        ...
        for (Exercise exercise in _selectedExercises) {
          sharedExercise
              .add(_service.shareExerciseTo(exercise, _currentUser.id, mobile));
        }
          ...
          if (err is DioError) {
            if (err.response.statusCode == 500) {
              _scaffoldKey.currentState.showSnackBar(
                  SnackBar(content: Text(err.response.data['message'])));
            } else {
              _scaffoldKey.currentState
                  .showSnackBar(SnackBar(content: Text("网络不给力啊！")));
            }
          }
        });
      }
