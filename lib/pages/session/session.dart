...
import 'package:workout_helper/pages/exercise/exercise_template_selection.dart';
import 'package:workout_helper/pages/exercise/save_template.dart';
...
import 'package:workout_helper/pages/general/flip_count_down_util.dart';
...
import 'package:workout_helper/pages/session/movement_bottom_sheet.dart';
import 'package:workout_helper/pages/session/session_action_menu.dart';
import 'package:workout_helper/pages/session/session_completed.dart';
import 'package:workout_helper/pages/session/session_materials_grid.dart';
...
import 'cardio_bottom_sheet.dart';
import 'hiit_bottom_sheet.dart';
import 'hiit_clock.dart';
import 'hiit_exercise_line.dart';
        ...
        actions: [
          SessionActionMenu(
            onSelected: (SessionOption option) {
              debugPrint('option' + option.toString());
              switch (option) {
...
//                  break;
                case SessionOption.CREATE_FROM_TEMPLATE:
                  ...
                  _service
                      .getUserExerciseTemplate(_currentUser.id)
  ...
  void _saveAsTemplate() async {
    if (_currentSession.matchingExercise.plannedSets.isEmpty) {
      _scaffoldKey.currentState
          .showSnackBar(SnackBar(content: Text("请先添加训练动作")));
      return;
    }
                    ...
                    children: <Widget>[
                      ...
                      FlatButton(
                        ...
                        onPressed: () {
                          ...
                          sessionService.removeSession(_currentSession);
  ...
  void initState() {
    super.initState();
    sessionService = SessionService(_scaffoldKey);
    ...
    _videoAnalysisService = VideoAudioAnalysisService(_scaffoldKey);
  }
  ...
  void didChangeDependencies() {
    ...
    SharedPreferences.getInstance().then((prefs) {
      ...
      if (currentSessionId == null) {
        sessionService
            .createNewSessionFromExercise(exercise, _currentUser.id.toString())
            .then((Session session) {
          setState(() {
            _currentSession = session;
          });
        }).catchError((error) {
          debugPrint("error in session:" + error.toString());
          Navigator.maybePop(context);
        });
      } else {
        sessionService.getSessionById(currentSessionId).then((session) {
  ...
  Widget buildContent(BuildContext context) {
    if (_currentSession == null ||
        _currentSession.matchingExercise.plannedSets.isEmpty) {
      return Center(
          child: Text(
        "通过\"+\"添加训练动作\n"
        "或者\n"
    ...
    maps.forEach((Movement movement, List<ExerciseSet> sets) {
      ...
      if (movement.exerciseType == ExerciseType.lifting) {
        ...
        panel = SessionUtil.getCardioPanel(
            movement,
            sets,
            _currentPanel == currentIndex,
            expansionPanelHeader(movement, sets));
      }
      movements.add(panel);
      currentIndex++;
    });
                  ...
                  Navigator.of(context).push(MaterialPageRoute(
                      builder: (context) => VideoRecorder(
                          ...
                          uploadCompleted: (uploadedAt) async {
                            ...
                            material.sessionId = _currentSession.id;
...
//                                "https://lifting-ren-user.oss-cn-hangzhou.aliyuncs.com/115093490613878784/raw_material/1574040282432918.mp4";
                                uploadedAt;
                            material.matchingMovement = movement;
  ...
  void handleExerciseComplete(ExerciseSet es, List<ExerciseSet> sets,
      {int startSecond = 0}) {
    bool isSameMovement() {
      return _preSetCes.accomplishedSet.runtimeType == es.runtimeType &&
          _preSetCes.accomplishedSet.extractMovementBasicInfo().id ==
              es.extractMovementBasicInfo().id;
    }
    ...
    SessionCompleted sc = SessionCompleted(
      ...
      savedRepeat:
          _preSetCes == null || !isSameMovement() ? null : _preSetCes.repeats,
      savedWeight:
          _preSetCes == null || !isSameMovement() ? null : _preSetCes.weight,
    );
  ...
  void doSubmitSetCompleted(List<ExerciseSet> sets, ExerciseSet es) {
    ...
    sessionService.saveCompletedSet(_currentSession, _preSetCes);
                    ...
                    builder: (context) {
                      return Center(
                        child: Container(
                            ...
                            child: Material(child: dialogWidget)),
                      );
                    }).then((_) {});
              },
            )),
...
//            animatedIconTheme: IconThemeData(size: 22.0),
                      ...
                      builder: (context) {
                        return CardioBottomSheet(logExercise: (ExerciseSet es) {
                          appendToCurrentSession([es]).then((_) {
                            ...
                            sessionService.saveCompletedSet(
                                _currentSession, ces);
                          });
                        });
                      })
                          ...
                          showDialog<String>(
                            ...
                            if (action == "OK") {
                              setState(() {
                                ...
                                sessionService
                                    .completedSession(_currentSession, userId)
                                    .then((_) {
                                  _currentSession.accomplishedTime = DateTime.now();
  ...
  Widget buildMaterialPart() {
      ...
      onPressed: () {
        Navigator.of(context).push(MaterialPageRoute(
            builder: (context) => CameraCapture(
                remoteStoreLocation: "session/" + _currentSession.id,
                fileUploaded: (String uploadedFile) {
                  ...
                  sessionMaterial.aspectRatio =
                      MediaQuery.of(context).size.width /
                          MediaQuery.of(context).size.height;
                  sessionMaterial.landscape =
                      MediaQuery.of(context).orientation ==
                          Orientation.landscape;
                  sessionService.addSessionMaterial(sessionMaterial);
                })));
      },
      color: Colors.transparent,
      child: Icon(
    ...
    return FutureBuilder<List<SessionMaterial>>(
      future: sessionService.getSessionMaterialsBySessionId(_currentSession.id),
        ...
        builder: (BuildContext context) {
          return HiitClock(
            ...
            exerciseRestLoopEndCallback: () {
              ...
              sessionService.saveCompletedSet(_currentSession, ces);
              Navigator.of(context).maybePop();
            },
          );
        });
