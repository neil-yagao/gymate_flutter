...
class MovementListPageState extends State<MovementListPage> {
  ...
  void onTapDown(BuildContext context, TapDownDetails details) {
    ...
    debugPrint('width:${box.size.width}');
    ...
    debugPrint('point(${originX},${originY})');
    debugPrint('selected muscle group:${muscleGroup}');
    if (muscleGroup != null) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        key: _scaffoldKey,
        appBar:DefaultAppBar.build(context,title:"训练动作"),
        body: SafeArea(
            child: Stack(
          children: [
            ListView(
...
//                        )),
  ...
  MuscleGroup determineWhichMuscle(double originX, double originY) {
    ///shoulder
    if (((originX >= 153 && originX <= 213) ||
            (originX >= 380 && originX <= 448)) &&
        ((originY >= 231 && originY <= 293) ||
            (originY >= 1217 && originY <= 1271))) {
    ...
    ///bicep
    if (((originX >= 140 && originX <= 195) ||
            (originX >= 385 && originX <= 437)) &&
    ...
    ///calfs
    if (((originY >= 760 && originY <= 900) ||
            (originY >= 1781 && originY <= 1875)) &&
        (originX >= 175 && originX <= 420)) {
      return MuscleGroup.CALVES;
    }

    ///lat
    if (((originX >= 208 && originX <= 269) ||
            (originX >= 341 && originX <= 384)) &&
        (originY >= 1316 && originY <= 1390)) {
      return MuscleGroup.LATS;
    }

    ///Triceps
    if (((originX >= 152 && originX <= 190) ||
            (originX >= 396 && originX <= 450)) &&
