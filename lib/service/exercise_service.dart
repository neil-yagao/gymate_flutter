...
class ExerciseService {
  ...
  Future<List<Exercise>> getUserExerciseTemplate(int userId) {
    return dio.get("/exercise/template/" + userId.toString()).then((
        Response r) {
  ...
  Future<List<ExerciseSet>> appendToExercise(Exercise exercise,
      List<ExerciseSet> ess) {
    ...
    List<ExerciseSet> giantSets = ess
      ...
      (es as GiantSet).movements.forEach((SingleMovementSet sms) {
        gs.movements.add(SingleMovementSet('', sms.sequence, sms.movement,
            sms.expectingRepeatsPerSet, sms.expectingWeight));
      });
      return gs;
    })?.toList();

    List<ExerciseSet> reduceSets = ess
    ...
    List<ExerciseSet> cardioSets = ess
    ...
    if(singleMovements.length > 0){
      futures.add(dio.put('/exercise/' + exercise.id + '/single_movements', data: singleMovements).then((Response r){
    ...
    if(reduceSets.length > 0){
      futures.add(dio.put('/exercise/' + exercise.id + '/reduce_movements', data: reduceSets).then((Response r){
        (r.data as List<dynamic>).forEach((value){
    ...
    if(giantSets.length > 0){
      futures.add(dio.put('/exercise/' + exercise.id + '/giant_movements', data: giantSets).then((Response r){
        (r.data as List<dynamic>).forEach((value){
    ...
    if(hiitSets.length > 0){
      futures.add(dio.put('/exercise/' + exercise.id + '/hiit_movements', data: hiitSets).then((Response r){
        (r.data as List<dynamic>).forEach((value){
    ...
    if(cardioSets.length > 0){
      futures.add(dio.put('/exercise/' + exercise.id + '/cardio_movements', data: cardioSets).then((Response r){
        (r.data as List<dynamic>).forEach((value){
  ...
  Future removeFromExercise(Exercise exercise, List<ExerciseSet> ess){
    return dio.put("/exercise/" + exercise.id  + "/sets",data: ess).then((_){
